 ...
 */
var createDonut = function(divid, userdata){
   
  var w = Math.min(window.innerWidth/2, 300),			// Width of the viz node
      h =  Math.min(window.innerWidth/2, 300),			// Height of the viz node
      r1 = Math.min(w, h) / 2,	// Outer radius of the donut
      r0 = r1 * .6,		// Inner radius of the donut
      n = 3,			// Number of donut segments
      q = 0,
      data0 = [0.5, 0.5, 0.5],
      data1 = [0.2, 0.1, 0.5],
  ...
  animateDonut(userdata);
    
}; 
function animateDonut(userData){
...
function submitForm() {
   var userData = [document.forms[0].elements[0].value,
...
function arcs(data0, data1) {
  var arcs0 = window.donut(data0),		// Its current position
      arcs1 = window.donut(data1),		// Its next position
      i = -1,
      arc;
